...
def inp(prompt, m=-1):
    ...
    Automatically handles translation between English and user's language.
    """
    global Mode
    if m == -1:
        m = Mode
    
    # Translate the prompt to user's language before displaying
    translated_prompt = translator.translate_to_user(prompt)
    print(translated_prompt)
    
    if m == 0:
        user_input = input()
        # Translate user input back to English for processing
        return translator.translate_to_english(user_input)
    else:
        vtt.record_audio("user_audio.wav")
        j = vtt.transcribe_audio("user_audio.wav")
        # Show what was recognized in the user's language
        print(f"{translator.translate_to_user('You said:')} {j}")
        # Translate the speech input to English for processing
        return translator.translate_to_english(j)
    ...
    while True:
        ...
        if validation_type == "frequency":
            ...
            if response.lower() in valid:
                ...
                print(translator.translate_to_user("Please provide one of the following responses: Rarely, Occasionally, Frequently, All the time."))
            ...
            try:
                ...
                if 1 <= severity <= 10:
                    ...
                    print(translator.translate_to_user("Please provide a number between 1 and 10."))
            except ValueError:
                print(translator.translate_to_user("Please provide a valid number between 1 and 10."))
        else:
            return response
...
def generate_markdown_report(data: dict) -> str:
    ...
    """
    report_lines = []
    report_lines.append("# " + translator.translate_to_user("Patient Summary Report"))
    report_lines.append("")
    report_lines.append("## " + translator.translate_to_user("Patient Details"))
    report_lines.append(f"- **{translator.translate_to_user('Patient ID')}:** {data.get('Patient ID', '')}")
    report_lines.append(f"- **{translator.translate_to_user('Name')}:** {data.get('Name', '')}")
    report_lines.append(f"- **{translator.translate_to_user('Age')}:** {data.get('Age', '')}")
    report_lines.append(f"- **{translator.translate_to_user('Gender')}:** {data.get('Gender', '')}")
    report_lines.append(f"- **{translator.translate_to_user('Phone')}:** {data.get('Phone', '')}")
    report_lines.append("")
    report_lines.append("## " + translator.translate_to_user("Medical History"))
    med_history = data.get("Medical History", {})
    for key, value in med_history.items():
        if isinstance(value, dict):
            response = value.get("response", "")
            details = value.get("details", "")
            line = f"- **{translator.translate_to_user(key)}:** {translator.translate_to_user(response.capitalize())}"
            if details:
                line += f" ({translator.translate_to_user('Details')}: {translator.translate_to_user(details)})"
            report_lines.append(line)
        else:
            report_lines.append(f"- **{translator.translate_to_user(key)}:** {translator.translate_to_user(value)}")
    report_lines.append("")
    report_lines.append("## " + translator.translate_to_user("Symptoms"))
    symptoms = data.get("Symptoms", {})
    if not symptoms:
        report_lines.append(translator.translate_to_user("No symptoms reported."))
    else:
        for symptom, details in symptoms.items():
            report_lines.append(f"- **{translator.translate_to_user(symptom.capitalize())}:**")
            if details and len(details) >= 3:
                frequency = details[0]
                severity = details[1]
                onset = details[2]
                report_lines.append(f"    - **{translator.translate_to_user('Frequency')}:** {translator.translate_to_user(frequency)}")
                report_lines.append(f"    - **{translator.translate_to_user('Severity')}:** {severity}")
                report_lines.append(f"    - **{translator.translate_to_user('Onset')}:** {translator.translate_to_user(onset)}")
            else:
                report_lines.append(f"    - {translator.translate_to_user('No details provided.')}")
    return "\n".join(report_lines)
...
class Patients:
    def __init__(self):
        ...
        try: 
            ...
            print(translator.translate_to_user(f"Database connection error: {e}"))
            return
    ...
    def add_patient(self):
        name = inp(translator.translate_to_user("Please input your full name:"), 0)
        num = inp(translator.translate_to_user("Please input your phone number (without the +91 extension and spaces):"), 0)
        while len(num) != 10 or not num.isnumeric():
            if num.strip().lower() == "skip":
                break
            num = inp(translator.translate_to_user("Please input a valid phone number or type \"skip\" to skip:"), 0)
        found, rec_ids = (False, [])
        if num.strip().lower() != "skip":
        ...
        if found:
            ...
            if patient is None:
                print(translator.translate_to_user("Error: Failed to fetch the record by id"))
                return 0, None
            rec_str = patient.rt_str()
            yn = inp(f"{translator.translate_to_user('This record was found associated with your phone number. Is this you?')} (y/n)\n{rec_str}", 0).strip().lower()
            while yn not in ["y", "n", "yes", "no"]:
                yn = inp(translator.translate_to_user("Please input a valid response (y/n):"), 0).strip().lower()
            if yn in ["y", "yes"]:
                return 1, pid
        gender = inp(translator.translate_to_user("Please input your gender (Male/Female):"), 0).strip().lower()
        while gender not in ["male", "female"]:
            gender = inp(translator.translate_to_user("Please provide a valid input (Male/Female):"), 0).strip().lower()
        age = inp(translator.translate_to_user("Please input your age:"), 0).strip()
        while not age.isnumeric():
            age = inp(translator.translate_to_user("Please provide a valid numeric age:"), 0).strip()
        password = inp(translator.translate_to_user("Please input your new password:"), 0).strip()
        confirm_password = inp(translator.translate_to_user("Please confirm your new password:"), 0).strip()
...
class Patient:
    ...
    def save(self,db):
        ...
        try:
            ...
            print(translator.translate_to_user(f"Error: {e}"))
            return False
    ...
    def rt_str(self):
        return f'''{translator.translate_to_user('ID')}: {self.id}
{translator.translate_to_user('Name')}: {self.name}
{translator.translate_to_user('Phone no')}: {self.Phone}
{translator.translate_to_user('Gender')}: {translator.translate_to_user(self.gender)}
{translator.translate_to_user('Age')}: {self.age}'''


def main1(patients_db):
    # Set up language preferences first

    doctororpatient = inp(translator.translate_to_user("Are you a doctor or a patient? (d/p):"), 0).strip().lower()
    while len(doctororpatient) < 1 or doctororpatient[0] not in ["d", "p", "doctor", "patient"]:
        doctororpatient = inp(translator.translate_to_user("Please input a valid response (d/p):"), 0).strip().lower()

    #login logic for doctors
    if doctororpatient in ["d", "doctor"]:
        printallrecords = inp(translator.translate_to_user("Do you want to print all records? (y/n):"), 0).strip().lower()
        while len(printallrecords) < 1 or printallrecords[0] not in ["y", "n", "yes", "no"]:
            printallrecords = inp(translator.translate_to_user("Please input a valid response (y/n):"), 0).strip().lower()
        if printallrecords in ["y", "yes"]:
            doctorhandler.print_patient_records()
            return 0
        else:
            print(translator.translate_to_user("Thank you for using our service"))
            return 0 

    #login logic for patients
    yn = inp(translator.translate_to_user("Is this your first time in this hospital? (y/n):"), 0).strip().lower()
    ...
    if yn in ["y", "yes"]:
        ...
        phone_no = inp(translator.translate_to_user("Please enter your registered phone number (without +91 and spaces):"), 0)
        password = inp(translator.translate_to_user("Please enter your password:"), 0)
        ...
        if found:
            ...
            while(patient.password != password):
                print(translator.translate_to_user("Incorrect password."))
                forgot_password = inp(translator.translate_to_user("Forgot your password? (y/n):"), 0).strip().lower()
                if forgot_password in ["y", "yes"]:
                    print(translator.translate_to_user("Please contact your doctor for password reset."))
                    return None
                elif forgot_password in ["n", "no"]:
                    password = inp(translator.translate_to_user("Please enter your password:"), 0)
                else:
                    print(translator.translate_to_user("Please enter a valid response."))

            if patient.password == password:
                print(translator.translate_to_user(f"Medical record pulled for Patient ID: {patient_id}"))
                return patient 
        else:
            print(translator.translate_to_user("No record found. Please register as a new patient."))
            new_patient, patient_id = patients_db.add_patient()
...
def Gather_info(patients_db, patient):
    ...
    major_surgery = inp(translator.translate_to_user("Have you undergone any major surgeries? (yes/no):"), 0).strip().lower()
    if major_surgery in ["yes", "y"]:
        surgery_details = inp(translator.translate_to_user("Please provide details of your major surgeries (e.g., type, year, outcome):"), 0)
    ...
    patient.history["family_history"] = inp(translator.translate_to_user("Do your parents or grandparents suffer from similar symptoms? (yes/no):"), 0).strip().lower()
    patient.history["allergies"] = inp(translator.translate_to_user("Do you have any known allergies? If yes, please specify. If no, type 'no':"), 0).strip()
    patient.history["smoker"] = inp(translator.translate_to_user("Are you a smoker? (yes/no):"), 0).strip().lower()
    patient.history["sexual_history"] = inp(translator.translate_to_user("Would you like to disclose any relevant sexual health information? (yes/no):"), 0).strip().lower()
    ...
    print(translator.translate_to_user("\nNow, please describe your symptoms in plain English."))
    print(translator.translate_to_user("For example: 'I feel pain in my head and I'm tired all the time.'"))
    print(translator.translate_to_user("When you are done, please provide your exit command."))
    
    # Allow multiple rounds of symptom input.
    matched_symptoms = []
    while True:
        if len(matched_symptoms) == 0:
            user_input = inp(translator.translate_to_user("Describe your symptoms:"), 0)
            if gemini.is_exit_command(user_input):
                print(translator.translate_to_user("Exit command recognized. Ending symptom input."))
                break
            ...
            if len(extracted_symptoms) == 1 and extracted_symptoms[0] == "no":
                print(translator.translate_to_user("No symptoms identified in your input. Please describe your symptoms in more detail."))
                continue
            ...
            if not matched_symptoms:
                print(translator.translate_to_user("None of the extracted symptoms matched our known symptoms. Please describe your symptoms in more detail."))
                continue
        ...
        if normalized_symptom in patient.history["symptoms"]:
            print(translator.translate_to_user(f"The symptom '{normalized_symptom}' has already been recorded. Skipping follow-up for this symptom."))
            continue
        ...
        freq_question = translator.translate_to_user(f"How often do you experience {normalized_symptom}? (Rarely, Occasionally, Frequently, All the time)")
        sev_question = translator.translate_to_user(f"On a scale of 1-10, how severe is your {normalized_symptom}?")
        start_question = translator.translate_to_user(f"When did the {normalized_symptom} start? (e.g., 2 days ago, 1 week ago)")
...
if __name__ == "__main__":
    ...
    a = inp(translator.translate_to_user("Would you like voice or text input? (v/t):"), 0)
    if a.strip().lower() == "v":
        Mode = 1
    else:
        Mode = 0
    print(translator.translate_to_user(f"Hello! Welcome to {hospital_name}. I'm here to assist with your check-in. Let's start by gathering some details."))
    try: 
        patient = main1(p)
